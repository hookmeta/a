...
do
	Library = {
		Open = true;
		Accent = Color3.fromRGB(175, 194, 223);
		Pages = {};
		Sections = {};
		Flags = {};
		UnNamedFlags = 0;
	...
	do
        function Library:GetDate()
            ...
            --
            local String = string.format("%s, %s %d, %d", Week, Month, Day, Year)
            --
            return String
        end
        --
	...
	do
		...
		function Library:Window(Options)
			local Window = {
                ...
                Animate = {
                    Enabled = Options.Animate.Enabled,
                    InverseSpeed = 0.2,
                    ReverseSpeed = 0.2
                },
			...
			--
			local ScreenGui = Instance.new("ScreenGui", game:GetService("RunService"):IsStudio() and game.Players.LocalPlayer.PlayerGui or gethui())
            ...
            if Window.Animate.Enabled then
                local function animateTitle()
                    while true do
                        for i = 1, #Window.Name do
                            Title.Text = Window.Name:sub(1, i)
                            wait(Window.Animate.InverseSpeed) 
                        end
            
                        wait(1)
            
                        for i = #Window.Name, 1, -1 do
                            Title.Text = Window.Name:sub(1, i - 1)
                            wait(Window.Animate.ReverseSpeed) 
                        end
